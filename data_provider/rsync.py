...
def get_stage_in_cmd(file, jumphost = None):
    if jumphost:
        ...
        # in the wrapper.
        cmd = "ssh -o ControlPath ~/.ssh/`hostname` -J {jumphost} {hostname} exit; rsync -avzq  -e 'ssh -J {jumphost}' {hostname}:{permanent_filepath} {worker_filepath}".format(
            jumphost = jumphost,
...
def get_stage_out_cmd(file, jumphost = None):
    if jumphost:
        ...
        # the special case we use subprocess.run(cmd, shell=True)
        # in the wrapper.
        cmd = "ssh -o ControlPath ~/.ssh/`hostname` -J {jumphost} {hostname} exit; rsync -avzq -e 'ssh -J {jumphost}' --rsync-path=\"mkdir -p {root_path} && rsync\" {worker_filepath} {hostname}:{permanent_filepath}".format(
            jumphost = jumphost,
            hostname = file.netloc,
            permanent_filepath = file.path,
            worker_filepath = file.local_path,
