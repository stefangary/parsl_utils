...
def get_stage_in_cmd(file, jumphost = None):
    if jumphost:
        ...
        cmd = "timeout 60 rsync -avzq -e 'ssh -J {jumphost}' {hostname}:{permanent_filepath} {worker_filepath}".format(
            jumphost = jumphost,
...
def get_stage_out_cmd(file, jumphost = None):
    if jumphost:
        ...
        cmd = "timeout 60 rsync -avzq -e 'ssh -J {jumphost}' --rsync-path=\"mkdir -p {root_path} && rsync\" {worker_filepath} {hostname}:{permanent_filepath}".format(
            jumphost = jumphost,
            hostname = file.netloc,
            permanent_filepath = file.path,
            worker_filepath = file.local_path,
